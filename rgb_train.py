...
from dataloaders.rgb_video import get_dataset_and_data_loader
from models.rgbvt5 import *
from utils.losses import get_loss_fn
from utils.rgb_trainer import train_one_epoch, evaluate_one_epoch, evaluate_one_epoch_sn
...
with open(f"{os.getcwd()}/rgb_configs/{args.config}") as config_file:
...
model = RGBVT5Model(config["rgbvt5_config"], spottting_loss_fn).to(device)
...
for epoch in range(config["epoches"]):
    ...
    train_one_epoch(
        model,
        train_dataloader,
        config["rgbvt5_config"]["short_memory_len"],
        config["rgbvt5_config"]["umt5_enabled"],

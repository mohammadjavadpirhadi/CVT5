...
class CompressedVideoDataset(Dataset):
    ...
    def __init__(self, splits, config):
        ...
        assert self.feature_extrator in ['resnet', 'clip'], "Feature extractor should be one of 'resnet' or 'clip'"
    ...
    def load_frames(self):
        ...
        for frames_path in tqdm(self.frames_pathes, desc="Scanning frames pathes"):
            ...
            for frame_path in tqdm(frame_pathes, desc="Frames", position=1, leave=False):
                if frame_path.endswith(f"I_{self.feature_extrator}.pt"):
    ...
    def get_chunk(self, video_index, frames_start_index):
        ...
        return {
            ...
            'I_frames': torch.stack([torch.load(I_frame) for I_frame in self.I_frames[video_index][I_frames_start_index:I_frames_start_index+I_frames_count]], dim=0) if I_frames_count.item() else torch.Tensor(0),
